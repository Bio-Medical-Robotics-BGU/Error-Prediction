...
from tensorflow.keras.layers import Dense, Input, Bidirectional, GRU, LayerNormalization
...
TestKinematics = np.squeeze(TestKinematics[:, :
                                           , inds])
...
for j in range(0, 5):
    ...
    x1 = Bidirectional(GRU(gru1, activation='tanh', return_sequences=True,
                            dropout=do, recurrent_dropout=rdo, kernel_regularizer=l2(reg)))(Input1)
    
    
    x2 = LayerNormalization(axis=-1, center=True, scale=True)(x1)
    
    x3 = Bidirectional(GRU(gru2, activation='tanh', return_sequences=False,
                            dropout=do, recurrent_dropout=rdo, kernel_regularizer=l2(reg)))(x2)
    
    x4 = LayerNormalization(axis=-1, center=True, scale=True)(x3)
    ...
    lr_scheduler = ReduceLROnPlateau(
        ...
        patience=3,           # epochs wi☺th no improvement before reducing
    ...
    os.chdir(project)
    model.save_weights(f'Weights_BiGRU_256_128_do0.2rcdo0.2_reg0.002_PSM_lr0.0001_bs128_cw2_e100_Test_rep{j}.weights.h5')
